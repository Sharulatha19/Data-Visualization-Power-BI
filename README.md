...
- Create DAX measures for KPIs (Sales, Profit, Target Achievement)
- Build a robust data model with relationships
- Visualize sales trends, category performance, and regional distribution
...
- **Data Volume:** 500 rows covering Jan–Nov 2026
...
- Converted text dates into proper Date type
...
Key Measures: Total Sales = SUM('Order Details'[Amount]) Total Profit = SUM('Order Details'[Profit]) Target Achievement % = DIVIDE([Total Sales], SUM('Sales Target'[Target])) Revenue per Order = DIVIDE([Total Sales], DISTINCTCOUNT('Orders'[Order ID])) YoY Growth % = DIVIDE([Total Sales] - [Prior Year Sales], [Prior Year Sales])
